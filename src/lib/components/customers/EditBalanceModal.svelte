<!-- src/lib/components/customers/EditBalanceModal.svelte -->
<script>
	...
	function validate() {
		...
		if (amount === '' || amount === null || isNaN(value) || value < 0) {
			errorMessage = 'Saldo wajib diisi dan tidak boleh negatif';
			return false;
		}
		return true;
	}
	...
	async function handleSubmit() {
		...
		isSubmitting = true;
		try {
			// PUT /customerbalance/set = MENIMPA saldo ke nilai ini (bukan menambah)
			await api.customerBalance.set({
			...
			toast.error(err.message || 'Gagal mengoreksi saldo');
	...
	<div class="flex min-h-screen items-center justify-center px-4 py-6">
		...
		<div class="relative w-full max-w-md transform rounded-lg bg-white shadow-xl transition-all">
			<div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
				<h3 class="text-lg font-medium text-gray-900">Koreksi Saldo</h3>
			...
			<div class="px-6 py-4">
				...
				<p class="mb-4 text-sm text-gray-500">
					Ini menimpa saldo langsung ke angka yang kamu masukkan (bukan menambah). Pakai ini buat
					koreksi salah input, atau isi 0 untuk mengosongkan saldo.
				</p>

				<form on:submit|preventDefault={handleSubmit} class="space-y-4">
					<div>
						<label for="edit-balance-amount" class="block text-sm font-medium text-gray-700">
							Saldo Baru <span class="text-red-500">*</span>
						</label>
						<CurrencyInput id="edit-balance-amount" bind:value={amount} placeholder="0" />
						{#if errorMessage}
